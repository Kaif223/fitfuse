...
export default function Feed() {
  ...
  const sendComment = async () => {
    ...
    try {
      const newComment = await commentsApi.addComment(activePostId, user.id, commentText.trim());
              ...
              return (
                <View key={p.id}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                      {p.user_avatar ? (
                        ...
                        <View style={styles.avatarFallback}>
                          <Text style={styles.avatarText}>{getInitials(p.user_name)}</Text>
                        </View>
                      )}
                      <View>
                  ...
                  <View style={styles.actions}>
                    ...
                    <Pressable>
                      <Bookmark size={20} color={colors.ink} strokeWidth={1.6} />
                    </Pressable>
                  </View>

                  {p.caption ? (
                    <Text style={styles.caption}>
                      <Text style={{ fontWeight: '600' }}>{p.user_name} </Text>
                      {p.caption}
                    </Text>
                  ) : null}
                </View>
            ...
            <Pressable style={styles.imagePicker} onPress={pickImage}>
              {newImageUri ? (
                ...
                <View style={{ alignItems: 'center', gap: 8 }}>
                  ...
                  <Text style={{ color: colors.inkSoft, fontSize: 14 }}>Tap to pick outfit photo</Text>
                </View>
              )}
            </Pressable>
            <TextInput
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
          ...
          {commentsLoading ? (
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <View style={styles.empty}>
                  <Text style={styles.emptySubtext}>No comments yet. Be the first!</Text>
                </View>
              }
              renderItem={({ item }) => (
...
with open('/home/kaif/Downloads/fitfuse-complete/fitfuse/app/(tabs)/feed.tsx', 'w') as f:
    f.write(content)
print("Done")
